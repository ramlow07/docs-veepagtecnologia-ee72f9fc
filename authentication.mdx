...
<Note>
  O CORS aceita headers como `Authorization`, `X-API-Key` e `X-Token`, mas a leitura desses headers pelo guard de autenticação não foi confirmada na codebase analisada. Para evitar surpresas, use `apiKey` ou `token`.
</Note>
