...
<Note>
  O payload de `transaction.created` pode trazer `subscription`, `product` e `client` dentro de `transaction`, quando esses dados estiverem disponíveis no fluxo de criação. No envio atual confirmado em código, `transaction.update` envia os campos base da transação. O objeto `charge` não é enviado dentro de `transaction` hoje.
</Note>
...
| `acquirer` | `object` | Dados do adquirente: `chargeId`, `id`, `integrationId`, `name`, `orderId`, `authorization`, `tid` e `paymentId`. |
| `amount` | `object` | Valor da transação: `currency` e `value`. |
| `boleto` | `object` ou `null` | Dados de boleto: `barcode`, `dueDate`, `url` e `links`. |
| `card` | `object` ou `null` | Dados do cartão: `firstDigits`, `lastDigits`, `binInfo`, `reversible`, `brand`, `holderName` e `expiry`. |
...
| `productId` | `string` | Identificador do produto vinculado. |
| `client` | `object` | Cliente vinculado enviado em `subscription.created`, quando disponível no fluxo. |
| `product` | `object` | Produto vinculado enviado em `subscription.created`, quando disponível no fluxo. O nome comercial fica em `product.product.title` e o preço fica em `product.product.price`, no formato cadastrado no produto. |
...
No evento `charge.update`, quando a cobrança atualizada fica com status `PAID`, o objeto `client` é enviado dentro de `charge` com os dados cadastrais do cliente, incluindo nome, e-mail e documento quando cadastrados.
...
| `card` | `object` ou `null` | Resultado de cartão: `acquirer`, `brand`, `message` e `code`. |
...
| `createdAt` | `string` | Data de criação em formato ISO. |
| `client` | `object` | Cliente vinculado enviado em `charge.update` quando a cobrança fica `PAID`. |

## Objetos relacionados

### `client`

O objeto `client` é enviado dentro de `transaction.created` quando o cliente está disponível no fluxo de criação, dentro de `subscription.created` quando a assinatura é criada e dentro de `charge.update` quando a cobrança fica `PAID`.
...
O objeto `product` é enviado hoje dentro de `transaction.created` quando o produto está disponível no fluxo de criação.
