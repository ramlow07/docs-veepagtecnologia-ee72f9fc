...
No envio atual confirmado, o service de assinatura não carrega `client` nem `product` dentro do objeto `subscription` antes de enviar o webhook.
...
O objeto `client` é enviado hoje dentro de `transaction.created` quando o cliente está disponível no fluxo de criação.
